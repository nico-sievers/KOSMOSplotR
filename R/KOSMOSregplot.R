...
#' @param control A sample that stands out of the experimental design, such as a harbour or fjord sample, and shall be ignored in the regression calculation and plot. Name the identifier from the "Mesocosm" or "Treat_Meso" column. The default is set with the style template automatically.
...
KOSMOSregplot=function(dataset=KOSMOStestdata,
                       ...
                       control=KOSMOScurrentControl,subset_data=FALSE,exclude_meso=FALSE,
  ...
  if(is.logical(ylimit) && ylimit==FALSE){
    ...
    if(headspace=="default"){if(length(categories)==1){headspace=0.15}else{headspace=0.3}}
