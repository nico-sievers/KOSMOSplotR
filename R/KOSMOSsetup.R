...
KOSMOSselect=function(experiment="kiel"){
  ...
  # because the kiel ones are not truly unique make a negative selection if there is quartz or side experiment in the query
  ...
  if(nchosen>1){
    ...
    else{assign("KOSMOScurrentStatscols",KOSMOScurrentStyletable[nmesos-1,"colourlist"],envir=.GlobalEnv)}
